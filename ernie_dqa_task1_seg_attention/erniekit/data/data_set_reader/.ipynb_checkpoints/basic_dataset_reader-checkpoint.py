...
class BasicDataSetReader(BaseDataSetReader):
    ...
    def __iter__(self):
        ...
        for epoch_index in range(self.config.epoch):
            ...
            for input_file in data_files:
                if 'split' not in input_file and 'new' not in input_file:continue
                examples = self.read_json_files(os.path.join(self.config.data_path, input_file))
    ...
    def read_json_files(self, file_path, quotechar=None):
        """读取明文文件
        :param file_path
        ...
        with open(file_path, "r", encoding='utf-8') as f:
            try:
                examples = []
                self.Example = namedtuple('Example', ['text_a'])
                for linenum, line in enumerate(f):
                    ...
                    data_dict['org_answer'] = sample.get('org_answer', '')
                    data_dict['answer_list'] = sample.get('answer_list', [])
                    data_dict['answer_start_list']= sample.get('answer_start_list', [])
                    data_dict['label'] =sample.get('label','')
                    example = self.Example(*[data_dict])
                    examples.append(example)

                return examples

            except Exception:
                logging.error("error in read json, maybe occur in linenum %s " % line_index)
    ...
    def pad_batch_records(self, batch_records):
        ...
        for index, key in enumerate(example._fields):
            ...
            try:
                if self.fields[index].join_calculation:
                    ...
                    return_list_ids.extend(id_list)
                    return_list_tokens.extend(token_list)
            except Exception:
                lines = ''
        ...
        if self.config.need_generate_examples:
            return (return_list_ids, return_list_tokens), batch_records
        else:
