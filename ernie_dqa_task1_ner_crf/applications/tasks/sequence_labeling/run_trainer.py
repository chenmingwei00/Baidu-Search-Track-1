...
import erniekit.data.data_set as data_set
from trainer.custom_dynamic_trainer import CustomDynamicTrainer
...
def dataset_reader_from_params(params_dict):
    ...
    dataset_reader = data_set.DataSet(params_dict)
...
def build_trainer(params_dict, dataset_reader, model, num_train_examples=0):
    ...
    #trainer_class = CustomDynamicTrainer 
...
if __name__ == "__main__":
    ...
    args.param_path='./examples/seqlab_ernie_fc_ch.json'
    param_dict = params.from_file(args.param_path)
    _params = params.replace_none(param_dict)

    # 记得import一下注册的模块
    register.import_modules()
    register.import_new_module("model", "ernie_fc_sequence_label")
