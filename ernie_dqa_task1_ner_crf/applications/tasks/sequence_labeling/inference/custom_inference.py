...
class CustomInference(BaseInference):
    ...
    def inference_batch(self):
        ...
        with open('./output/4001.txt','w',encoding='utf-8') as ner_pre:
            for batch_id, data in enumerate(dg()):
                ...
                # 回调给解析函数
                write_result_list, write_score_list,write_ner_results = self.parser_handler(predict_results,
                        input_list=data_tokens, params_dict=self.params, seq_lens=seq_lens)
                for score, result in zip(write_score_list, write_result_list):
                    output_file.write(str(score) + '\t' + result + '\n')
                
                for t_ner_pre in write_ner_results:
