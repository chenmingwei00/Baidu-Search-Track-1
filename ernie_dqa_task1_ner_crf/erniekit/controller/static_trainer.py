...
class PGD:
    ...
    def attack(self, emb_name='embedding', is_first_attack=False, params_grads=None):
        k_step = 0
        for name, param in params_grads:
            if param.stop_gradient == False and emb_name in name.name:
                if is_first_attack:
                    self.emb_backup[name.name] = param.clone()
                norm = paddle.norm(param)
                r_at = self.alpha * param / norm
                param.add_(r_at)
                param = self.project(name, param)
                self.model.parameters()[k_step] = param
            k_step += 1
    ...
    def project(self, param_name, param_data):
        r = param_data - self.emb_backup[param_name.name]
        if paddle.norm(r) > self.eps:
            r = self.eps * r / paddle.norm(r)
        return self.emb_backup[param_name.name] + r

    def backup_grad(self, params_grads):
        for name, param in params_grads:
            if param.stop_gradient == False and param.gradient:
                self.grad_backup[name.name] = param.clone()

    def clear_gradets(self):
        for name, param in self.model.named_parameters():
            if param.stop_gradient == False and param.gradient:
                param.gradient = 0
...
class AWP:
    """
    implements weighted advserial perturbation
    """
    ...
    def attack_backward(self, params_grads):
        if self.adv_lr == 0:
            return
        self._save()
        self._attck_step(params_grads)

    def _attck_step(self,params_grads):
        e = 1e-6
        k_step=0
        for param,grad in params_grads:
            if param.stop_gradient == False and self.adv_param in param.name:
                norm1 = paddle.norm(grad)
                norm2 = paddle.norm(param)
                # 在损失之前获取梯度
                r_at = self.adv_lr * grad / (norm1 + e) * (norm2 + e)
                assert self.model.parameters()[k_step].name==param.name
                self.model.parameters()[k_step] = param + r_at
                self.model.parameters()[k_step] = paddle.minimum(
                    paddle.maximum(self.model.parameters()[k_step], self.backup_eps[param.name][0]), self.backup_eps[param.name][1]
                )
            k_step+=1
    ...
    def _save(self):
        for name, param in self.model.named_parameters():
            if param.stop_gradient==False and self.adv_param in param.name:
                if param.name not in self.backup:
                    self.backup[param.name] = param.clone()
                    grad_eps = self.adv_eps * param.abs().detach()
                    self.backup_eps[param.name] = (
                        self.backup[param.name] - grad_eps,
                        self.backup[param.name] + grad_eps,
                    )
    ...
    def _restore(self, ):
        k_step=0
        for name, param in self.model.named_parameters():
            if param.name in self.backup:
                assert self.model.parameters()[k_step].name==param.name
                self.model.parameters()[k_step] = self.backup[param.name]
            k_step+=1
        self.backup = {}
        self.backup_eps = {}
...
class BaseStaticTrainer(object):
    def __init__(self, params, data_set_reader, model, pgd_k=3):
        """
        :param params
        ...
        self.pgd = PGD(model=self.model_class)
        self.pgd_k = pgd_k
        self.awp = AWP(self.model_class,adv_param='embedding', adv_lr=0.8, adv_eps=0.001)
        # 参数解析
        # 动态图or静态图
    ...
    def _scale_l2(self, x, norm_length):
        ...
        alpha = paddle.fluid.layers.reduce_max(paddle.fluid.layers.abs(x), dim=[0, 1], keep_dim=True) + 1e-12
        l2_norm = alpha * paddle.fluid.layers.sqrt(
            paddle.fluid.layers.reduce_sum(paddle.fluid.layers.pow(x / alpha, 2), dim=[0, 1], keep_dim=True) + 1e-6)
        x_unit = x / l2_norm
        return norm_length * x_unit

    def adversarial_loss(self, loss, fields_dict, phase):
        grad = paddle.fluid.backward.append_backward(loss, [self.model_class.parameters()[2].name])[0][1]
        perturb = self._scale_l2(grad, 1.0)
        # adv_loss = self.cl_loss_from_embedding(self.model_class.parameters()[2] + perturb)  # 报错在这里
        # 如果交换位置，变成 perturb + self.feature 就不报错
        self.model_class.parameters()[2] = perturb + self.model_class.parameters()[2]
        self.forward_train_output = self.model_class.forward(fields_dict, phase=InstanceName.TRAINING)
        adv_loss = self.forward_train_output[InstanceName.LOSS]
    ...
    def init_static_train_net(self):
        ...
        with static.program_guard(self.train_program, self.startup_program):
            with paddle.fluid.unique_name.guard():
                ...
                # loss对抗训练,使用的FGM进行对抗训练
                old_word_embedding=self.model_class.parameters()[2].clone()
                new_loss=self.adversarial_loss(loss,fields_dict,InstanceName.TRAINING)
                self.optimizer.minimize(new_loss)
                self.model_class.parameters()[2]= old_word_embedding
                ###########################
                ...
                # init_parags=[[ele[0],ele[1]] for ele in params_grads]
                # self.pgd.backup_grad(params_grads) #原始参数保存
                # for k in range(self.pgd_k):
                #     self.pgd.attack(is_first_attack=(k == 0),params_grads=params_grads)
                #     if k != self.pgd_k - 1:
                #         self.pgd.clear_gradets()
                #     else:
                #         self.pgd.restore_grad()
                #     # loss_adv = model(batch)
                #     # loss_adv.backward()
                #     self.forward_train_output = self.model_class.forward(fields_dict, phase=InstanceName.TRAINING)
                #     pgd_loss = self.forward_train_output[InstanceName.LOSS]
                #     params_grads=self.optimizer.backward(pgd_loss)
                #
                #     indx=0
                #     for ele1,ele2 in zip(init_parags,params_grads):
                #         init_parags[indx][1]=ele1[1]+ele2[1]
                #
                # self.pgd.restore_grad()
                # self.optimizer.apply_gradients(init_parags)
                # PGD对抗训练代码#################################

                # AWP对抗训练代码#################################
                # self.forward_train_output = self.model_class.forward(fields_dict, phase=InstanceName.TRAINING)
                # loss = self.forward_train_output[InstanceName.LOSS]
                # params_grads = self.optimizer.backward(loss)  # 反向传播计算梯度
                # self.awp.attack_backward(params_grads)  #对参数进行攻击
                # #按照攻击之后的参数求得损失
                # self.forward_train_output = self.model_class.forward(fields_dict, phase=InstanceName.TRAINING)
                # new_loss = self.forward_train_output[InstanceName.LOSS]
                # params_grads = self.optimizer.backward(new_loss)
                # self.awp._restore()
