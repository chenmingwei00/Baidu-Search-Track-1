...
def truncation_words(words,answer_list,answer_start_list,label,max_seq_length, truncation_type):
    """
    :param words:
    ...
    words1 = words[:max_seq_length]
    words2 = words[max_seq_length:max_seq_length * 2]
    label1 = label[:max_seq_length]
    label2 = label[max_seq_length:max_seq_length * 2]
    if len(answer_list)!=0:

        for t_ans,t_ans_start in zip(answer_list,answer_start_list):
            if t_ans_start+len(t_ans)<len(words1): #说明答案都在前一个 doc1中
                answer_start_list1.append(t_ans_start)
                answer_list1.append(t_ans)
            elif t_ans_start<len(words1) and t_ans_start+len(t_ans)>len(words1): #说明当前答案被截断分为doc1和doc2中
                answer_start_list1.append(t_ans_start)
                answer_list1.append(''.join(words[t_ans_start:len(words1)]))
                answer_start_list2.append(0)
                answer_list2.append(''.join(words[len(words1):t_ans_start+len(t_ans)]))
            elif t_ans_start>max_seq_length*2:
                continue
            else:#说明答案再第二个doc2中
                t_ans_start=t_ans_start-len(words1)
                answer_start_list2.append(t_ans_start)
                answer_list2.append(t_ans)
    return words1,words2,answer_list1,answer_list2,answer_start_list1,answer_start_list2,label1,label2
