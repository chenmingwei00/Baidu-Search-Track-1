...
class ErnieFcSeqLabel(BaseModel):
    ...
    def structure(self):
        ...
        self.value_layer2 = paddle.nn.Linear(in_features=self.hid_dim, out_features=self.hid_dim,
                                            weight_attr=paddle.ParamAttr(name='value2_out_w',
                                                                         initializer=initializer),
                                            bias_attr='value2_out_b')
    def net_forward(self,
        ...
        _, tokens_embedding = self.ernie_model(src_ids=text_src, sent_ids=text_sent,
                                               task_ids=text_task, pos_ids=postion_ids)

        emb_text = self.dropout(tokens_embedding)

        if first_embedding:
            # pre attetion b 4 768
            self.key_embed = self.key_layer(first_embedding)
            self.value_embed = self.value_layer(first_embedding) #b,4,768

            self.query_embed = self.query_layer(emb_text)
            value2_embed=self.value_layer2(emb_text)
            key_embed=paddle.transpose(self.key_embed,perm=[0,2,1]) #b 768 4
            attention_scores = paddle.matmul(self.query_embed, key_embed)
            attention_scores = attention_scores * 1.0 / math.sqrt(self.hid_dim) #b,3,4
            attention_scores=paddle.nn.functional.softmax(attention_scores,axis=-1)
            key_value=paddle.matmul(attention_scores, self.value_embed) #b,3,768
            value_embed_new = key_value+value2_embed
        ...
        return loss,probs_start,probs_end,probs_ner,probs_crf,emb_text
    ...
    def forward(self, fields_dict, phase):
        ...
        pos_ids_text = input_dict['pos_ids']
        pos_ids2_text = input_dict['pos_ids2']
        ...
        # tokens_embedding = [batch_size, max_seq_len, hidden_size]
        text_task_ew=paddle.unsqueeze(text_mask,axis=-1)
        cls_embed=cls_embed*text_task_ew
        ...
        if phase == InstanceName.SAVE_INFERENCE:
            ...
            target_feed_list = [text_src, text_sent, text_mask,pos_ids_text ,text_src2, text_sent2, text_mask2,pos_ids2_text]
            target_feed_name_list = ["text_a#src_ids", "text_a#sent_ids", "text_a#mask_ids","text_a#pos_ids",
                                     "text_a#src_ids2", "text_a#sent_ids2", "text_a#mask_ids2","text_a#pos_ids2"]
